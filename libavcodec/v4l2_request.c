...
static int v4l2_request_probe_video_device(struct udev_device *device, AVCodecContext *avctx,
                                            ...
                                            struct v4l2_ext_control *control, int count)
{
    ...
    if (ctx->video_fd < 0) {
        av_log(avctx, AV_LOG_ERROR, "%s: opening %s failed, %s (%d)\n",
                __func__, path, av_err2str(AVERROR(errno)), errno);
        ret = AVERROR(EINVAL);
        goto fail;
    }
...
static int v4l2_request_probe_media_device(struct udev_device *device, AVCodecContext *avctx,
                                            uint32_t pixelformat, uint32_t buffersize,
                                            struct v4l2_ext_control *control, int count)
{
    V4L2RequestContext *ctx = avctx->internal->hwaccel_priv_data;
...
int ff_v4l2_request_init(AVCodecContext *avctx, uint32_t pixelformat, uint32_t buffersize,
                          struct v4l2_ext_control *control, int count)
{
    V4L2RequestContext *ctx = avctx->internal->hwaccel_priv_data;
    int ret = AVERROR(EINVAL);
