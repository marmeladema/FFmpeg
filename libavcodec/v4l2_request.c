                                          ...
                                          const uint8_t *data, uint32_t size)
{
    ...
    if (req->output.used + size + OUTPUT_BUFFER_PADDING_SIZE <= req->output.size) {
        ...
        av_log(avctx, AV_LOG_DEBUG, "%s: output.used=%u output.size=%u size=%u\n",
               __func__, req->output.used, req->output.size, size);
    }
    return 0;
}

static int v4l2_request_controls(V4L2DeviceVideo *device, int request_fd,
                                  unsigned long type, struct v4l2_ext_control *control, int count)
{
    ...
    return ioctl(device->fd, type, &controls);
}

static int v4l2_request_set_controls(V4L2DeviceVideo *device, int request_fd,
                                      struct v4l2_ext_control *control, int count)
{
    return v4l2_request_controls(device, request_fd, VIDIOC_S_EXT_CTRLS, control, count);
}
...
int ff_v4l2_request_set_controls(AVCodecContext *avctx, struct v4l2_ext_control *control, int count)
{
    ...
    ret = v4l2_request_controls(&ctx->video_device, -1, VIDIOC_S_EXT_CTRLS, control, count);
    if (ret < 0) {
...
int ff_v4l2_request_get_controls(AVCodecContext *avctx, struct v4l2_ext_control *control, int count)
{
    ...
    ret = v4l2_request_controls(&ctx->video_device, -1, VIDIOC_G_EXT_CTRLS, control, count);
    if (ret < 0) {
...
int ff_v4l2_request_query_control(AVCodecContext *avctx, struct v4l2_query_ext_ctrl *control)
{
    ...
    ret = ioctl(ctx->video_device.fd, VIDIOC_QUERY_EXT_CTRL, control);
    if (ret < 0) {
...
int ff_v4l2_request_query_control_default_value(AVCodecContext *avctx, uint32_t id)
{
    ...
    ret = ioctl(ctx->video_device.fd, VIDIOC_QUERYCTRL, &control);
    if (ret < 0) {
        av_log(avctx, AV_LOG_ERROR, "%s: query control failed, %s (%d)\n", __func__, av_err2str(AVERROR(errno)), errno);
        return AVERROR(errno);
    }
                                      ...
                                      V4L2RequestBuffer *buf, uint32_t flags)
{
    ...
    return ioctl(ctx->video_device.fd, VIDIOC_QBUF, &buffer);
}
...
static int v4l2_request_dequeue_buffer(V4L2RequestContext *ctx, V4L2RequestBuffer *buf)
{
    ...
    ret = ioctl(ctx->video_device.fd, VIDIOC_DQBUF, &buffer);
    if (ret < 0)
        return ret;
...
static int v4l2_request_queue_decode(AVCodecContext *avctx, AVFrame *frame, struct v4l2_ext_control *control,
                                      int count, int first_slice, int last_slice)
{
    V4L2RequestContext *ctx = avctx->internal->hwaccel_priv_data;
    V4L2RequestDescriptor *req = (V4L2RequestDescriptor*)frame->data[0];
    ...
    ret = v4l2_request_set_controls(&ctx->video_device, req->request_fd, control, count);
    if (ret < 0) {
        av_log(avctx, AV_LOG_ERROR, "%s: set controls failed for request %d, %s (%d)\n",
               __func__, req->request_fd, av_err2str(AVERROR(errno)), errno);
        return AVERROR(errno);
    }
    ...
    if (ret < 0) {
        av_log(avctx, AV_LOG_ERROR, "%s: queue output buffer %d failed for request %d, %s (%d)\n",
               __func__, req->output.index, req->request_fd, av_err2str(AVERROR(errno)), errno);
        return AVERROR(errno);
    }
    ...
    if (first_slice) {
        ...
        if (ret < 0) {
            av_log(avctx, AV_LOG_ERROR, "%s: queue capture buffer %d failed for request %d, %s (%d)\n",
                   __func__, req->capture.index, req->request_fd, av_err2str(AVERROR(errno)), errno);
            return AVERROR(errno);
        }
    }
    ...
    if (last_slice) {
        ret = v4l2_request_set_drm_descriptor(req, &ctx->format);
        if(ret < 0)
...
int ff_v4l2_request_decode_slice(AVCodecContext *avctx, AVFrame *frame,
                                  struct v4l2_ext_control *control, int count,
                                  int first_slice, int last_slice)
{
    V4L2RequestDescriptor *req = (V4L2RequestDescriptor*)frame->data[0];
...
static int v4l2_request_try_format(AVCodecContext *avctx, V4L2DeviceVideo *device, enum v4l2_buf_type type, uint32_t pixelformat)
{
    ...
    if (V4L2_TYPE_IS_OUTPUT(type)) {
        ...
        if (ioctl(device->fd, VIDIOC_CREATE_BUFS, &buffers) < 0) {
            av_log(avctx, AV_LOG_ERROR, "%s: create buffers failed for type %u, %s (%d)\n",
                   __func__, type, av_err2str(AVERROR(errno)), errno);
            return -1;
        }
    ...
    while (ioctl(device->fd, VIDIOC_ENUM_FMT, &fmtdesc) >= 0) {
...
static int v4l2_request_set_format(V4L2DeviceVideo *device, enum v4l2_buf_type type,
...
static int v4l2_request_select_capture_format(AVCodecContext *avctx, V4L2DeviceVideo *device)
{
    V4L2RequestContext *ctx = avctx->internal->hwaccel_priv_data;
    ...
    if (ioctl(ctx->video_device.fd, VIDIOC_G_FMT, &format) < 0) {
    ...
    while (ioctl(ctx->video_device.fd, VIDIOC_ENUM_FMT, &fmtdesc) >= 0) {
        for (i = 0; i < FF_ARRAY_ELEMS(v4l2_request_capture_pixelformats); i++) {
    ...
    for (int i = 0; i < FF_ARRAY_ELEMS(v4l2_request_capture_pixelformats); i++) {
        ...
        if (!v4l2_request_try_format(avctx, device, type, pixelformat))
            return v4l2_request_set_format(device, type, avctx->coded_width, avctx->coded_height, pixelformat, 0);
    }
#endif
...
static bool check_video_device(V4L2DeviceVideo *device, void *opaque)
{
    V4L2RequestParams *params = opaque;
    AVCodecContext *avctx = params->avctx;
    V4L2RequestContext *ctx = avctx->internal->hwaccel_priv_data;
    int ret;
    unsigned int capabilities = 0;

    if (device->capability.capabilities & V4L2_CAP_DEVICE_CAPS)
        capabilities = device->capability.device_caps;
    else
        capabilities = device->capability.capabilities;

    av_log(avctx, AV_LOG_DEBUG, "%s: avctx=%p ctx=%p path=%s capabilities=%u\n",
            __func__, avctx, ctx, device->devname, capabilities);
    ...
    ret = v4l2_request_try_format(avctx, device, ctx->output_type, params->pixelformat);
    if (ret < 0) {
        av_log(avctx, AV_LOG_WARNING, "%s: try output format failed\n", __func__);
        return false;
    }

    ret = v4l2_request_set_format(device, ctx->output_type, avctx->coded_width, avctx->coded_height, params->pixelformat, params->buffersize);
    if (ret < 0) {
    ...
    ret = v4l2_request_set_controls(device, -1, params->ptr_controls, params->num_controls);
    if (ret < 0) {
        av_log(avctx, AV_LOG_ERROR, "%s: set controls failed, %s (%d)\n",
                __func__, av_err2str(AVERROR(errno)), errno);
        return false;
    ...
    ret = v4l2_request_select_capture_format(avctx, device);
    if (ret < 0) {
...
static int v4l2_request_init_context(AVCodecContext *avctx)
{
    ...
    ret = ioctl(ctx->video_device.fd, VIDIOC_G_FMT, &ctx->format);
    if (ret < 0) {
        av_log(avctx, AV_LOG_ERROR, "%s: get capture format failed, %s (%d)\n",
               __func__, av_err2str(AVERROR(errno)), errno);
        ret = AVERROR(EINVAL);
    ...
    if (V4L2_TYPE_IS_MULTIPLANAR(ctx->format.type)) {
        av_log(avctx, AV_LOG_DEBUG,
               "%s: pixelformat=%d width=%u height=%u bytesperline=%u sizeimage=%u num_planes=%u\n",
               __func__, ctx->format.fmt.pix_mp.pixelformat,
               ctx->format.fmt.pix_mp.width, ctx->format.fmt.pix_mp.height,
               ctx->format.fmt.pix_mp.plane_fmt[0].bytesperline, ctx->format.fmt.pix_mp.plane_fmt[0].sizeimage,
               ctx->format.fmt.pix_mp.num_planes);
    } else {
        av_log(avctx, AV_LOG_DEBUG,
               "%s: pixelformat=%d width=%u height=%u bytesperline=%u sizeimage=%u\n",
               __func__, ctx->format.fmt.pix.pixelformat,
               ctx->format.fmt.pix.width, ctx->format.fmt.pix.height,
               ctx->format.fmt.pix.bytesperline, ctx->format.fmt.pix.sizeimage);
    }
    ...
    ret = ioctl(ctx->video_device.fd, VIDIOC_STREAMON, &ctx->output_type);
    if (ret < 0) {
    ...
    ret = ioctl(ctx->video_device.fd, VIDIOC_STREAMON, &ctx->format.type);
    if (ret < 0) {
...
int ff_v4l2_request_init(V4L2RequestParams params)
{
    AVCodecContext *avctx = params.avctx;
    V4L2RequestContext *ctx = avctx->internal->hwaccel_priv_data;
    ...
    ctx->media_device.fd = -1;
    ctx->video_device.fd = -1;
    ctx->timestamp = 0;

    ret = ff_v4l2_device_probe_video(avctx, &ctx->video_device, &params, check_video_device);
    if (ret)
        return ret;

    ret = ff_v4l2_device_retrieve_media(avctx, &ctx->video_device, &ctx->media_device);
    if (ret)
        return ret;
...
int ff_v4l2_request_uninit(AVCodecContext *avctx)
{
    ...
    if (ctx->video_device.fd >= 0) {
        ret = ioctl(ctx->video_device.fd, VIDIOC_STREAMOFF, &ctx->output_type);
        if (ret < 0)
            av_log(avctx, AV_LOG_ERROR, "%s: output stream off failed, %s (%d)\n",
                   __func__, av_err2str(AVERROR(errno)), errno);

        ret = ioctl(ctx->video_device.fd, VIDIOC_STREAMOFF, &ctx->format.type);
        if (ret < 0)
    ...
    if (ctx->video_device.fd >= 0)
        close(ctx->video_device.fd);

    if (ctx->media_device.fd >= 0)
        close(ctx->media_device.fd);
...
static int v4l2_request_buffer_alloc(AVCodecContext *avctx, V4L2RequestBuffer *buf, enum v4l2_buf_type type)
{
    ...
    ret = ioctl(ctx->video_device.fd, VIDIOC_G_FMT, &buffers.format);
    if (ret < 0) {
    ...
    if (V4L2_TYPE_IS_MULTIPLANAR(buffers.format.type)) {
        av_log(avctx, AV_LOG_DEBUG,
               "%s: pixelformat=%d width=%u height=%u bytesperline=%u sizeimage=%u num_planes=%u\n",
               __func__, buffers.format.fmt.pix_mp.pixelformat,
               buffers.format.fmt.pix_mp.width, buffers.format.fmt.pix_mp.height,
               buffers.format.fmt.pix_mp.plane_fmt[0].bytesperline, buffers.format.fmt.pix_mp.plane_fmt[0].sizeimage,
               buffers.format.fmt.pix_mp.num_planes);
    } else {
        av_log(avctx, AV_LOG_DEBUG, "%s: pixelformat=%d width=%u height=%u bytesperline=%u sizeimage=%u\n",
               __func__, buffers.format.fmt.pix.pixelformat,
               buffers.format.fmt.pix.width, buffers.format.fmt.pix.height,
               buffers.format.fmt.pix.bytesperline, buffers.format.fmt.pix.sizeimage);
    }

    ret = ioctl(ctx->video_device.fd, VIDIOC_CREATE_BUFS, &buffers);
    if (ret < 0) {
        av_log(avctx, AV_LOG_ERROR, "%s: create buffers failed for type %u, %s (%d)\n",
               __func__, type, av_err2str(AVERROR(errno)), errno);
        return ret;
    ...
    ret = ioctl(ctx->video_device.fd, VIDIOC_QUERYBUF, &buf->buffer);
    if (ret < 0) {
        av_log(avctx, AV_LOG_ERROR, "%s: query buffer %d failed, %s (%d)\n",
               __func__, buf->index, av_err2str(AVERROR(errno)), errno);
        return ret;
    }

    if (V4L2_TYPE_IS_OUTPUT(type)) {
        off_t offset = V4L2_TYPE_IS_MULTIPLANAR(type) ? buf->buffer.m.planes[0].m.mem_offset : buf->buffer.m.offset;
        void *addr = mmap(NULL, buf->size, PROT_READ | PROT_WRITE, MAP_SHARED, ctx->video_device.fd, offset);
        if (addr == MAP_FAILED) {
        ...
        ret = ioctl(ctx->video_device.fd, VIDIOC_EXPBUF, &exportbuffer);
        if (ret < 0) {
...
static AVBufferRef *v4l2_request_frame_alloc(void *opaque, size_t size)
{
    ...
    ret = ioctl(ctx->media_device.fd, MEDIA_IOC_REQUEST_ALLOC, &req->request_fd);
    if (ret < 0) {
